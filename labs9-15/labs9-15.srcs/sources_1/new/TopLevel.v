...
module TopLevel(Clk, Reset);
	...
	OrGate JrOrGate(.orinput1(AndOutput), .orinput2(JrMEM), .oroutput(OrOutput));
    Mux32Bit3to1 PCSrcMux(.out(PCIn), .inA(PCAddResult), .inB(JrMuxOutMEM), .inC(JumpInstructionMEM), .sel({JumpMEM, OrOutput}));
    ShifterID Shift_jr(.shiftinput(InstructionIF[25:0]), .shiftoutput(JumpInstruction));
    ...
    RegIF_ID IF_ID(.Clk(Clk), .Reset(Reset), .PC_4Input(PCAddResult), .PC_4Output(PCAddID), .Inst_input(InstructionIF), .Inst_output(InstructionID), .JumpInst_input({PCAddResult[31:28], JumpInstruction}), .JumpInst_output(JumpInstructionID));
    ...
    wire RegWriteWB;
    RegisterFile RegFile(.ReadRegister1(InstructionID[25:21]), .ReadRegister2(InstructionID[20:16]), .WriteRegister(RegDstMuxWB), .WriteData(WriteDataWB), .RegWrite(RegWriteWB), .Clk(Clk), .Reset(Reset), .ReadData1(ReadData1ID), .ReadData2(ReadData2ID));
...
.ReadData1Out(ReadData1EX), .ReadData2In(ReadData2ID), .ReadData2Out(ReadData2EX), .ZeroExtendIn(ZeroExtendID), .ZeroExtendOut(ZeroExtendEX), .SignExtendIn(SignExtendID), .SignExtendOut(SignExtendEX), .JumpIn(JumpID), .JumpOut(JumpEX), .JumpInst_input(JumpInstructionID),
.JumpInst_output(JumpInstructionEX));
	...
	AndGate Gate2(.andinput1(ZeroEX), .andinput2(MovEX), .andoutput(Gate2Out));
	Mux32Bit2To1 MovMuxEx(.out(ActualALUOutput), .inA(ALUResultEX), .inB(ReadData1EX), .sel(Gate2Out));
...
.MemReadIn(MemReadEX), .MemReadOut(MemReadMEM), .MemtoRegIn(MemtoRegEX), .MemtoRegOut(MemtoRegMEM), .MovIn(MovEX), .MovOut(MovMEM), .wordhalfbyteIn(wordhalfbyteEX), .wordhalfbyteOut(wordhalfbyteMEM),
 .PCAddIn(PCAddEX), .PCAddOut(PCAddMEM),.ReadData2In(ReadData2EX), .ReadData2Out(ReadData2MEM), .ZeroFlagIn(ZeroEX), .ZeroFlagOut(ZeroMEM), .ALUResultIn(ActualALUOutput), .ALUResultOut(ALUResultMEM),
 .RegDstMuxIn(RegDstMuxEX), .RegDstMuxOut(RegDstMuxMEM), .JrMuxIn(JrMuxOutEX), .JrMuxOut(JrMuxOutMEM), .JumpIn(JumpEX), .JumpOut(JumpMEM), .JumpInst_input(JumpInstructionEX), .JumpInst_output(JumpInstructionMEM), .JrIn(JrEX), .JrOut(JrMEM));
    
	//This is the memory stage
	wire [31:0] storeTreaterOut, loadTreaterOut;
	
	StoreMasker storeTreater(.ALUOutput(ReadData2MEM), .ReadData(ReadDataMEM), .wordhalfbyte(wordhalfbyteMEM), .LSBAddress(ALUResultMEM[1:0]), .WriteData(storeTreaterOut));
    DataMemory Data(.Address(ALUResultMEM), .WriteData(storeTreaterOut), .Clk(Clk), .Rst(Reset), .MemWrite(MemWriteMEM), .MemRead(MemReadMEM), .ReadData(ReadDataMEM));
	WordMask loadTreater(.information(ReadDataMEM), .wordhalfbyte(wordhalfbyteMEM), .LSBAddress(ALUResultMEM[1:0]), .finalInformation(loadTreaterOut));
    ...
    RegMEM_WB MEM_WB(.Clk(Clk), .Reset(Reset), .RegWriteIn(MovMuxOut),
     .RegWriteOut(RegWriteWB), .MemtoRegIn(MemtoRegMEM), .MemtoRegOut(MemtoRegWB),
      .PCAddIn(PCAddMEM), .PCAddOut(PCAddWB), .ALUResultIn(ALUResultMEM),
       .ALUResultOut(ALUResultWB), .ReadDataMemIn(loadTreaterOut),
