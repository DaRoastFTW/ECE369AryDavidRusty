...
loop_MaxIndex:
	bge		$t1, $a1, done_MaxIndex	# i >= n?
	mul		$t2, $t1, 4		# address i
	add		$t2, $t2, $a0	# address V[i]
	lw		$t2, 0($t2)		# V[i]
	ble		$t2, $t0, next_MaxIndex	# v[i] <= max?
	add		$t0, $t2, $0	# max = V[i]
...
sloop:
    ...
    sw  $t3, 0($t0)       # [V[$t1]=$t2] Store V[n-1] to be V[MaxIndex]
    sw  $t2, 0($t1)       # [V[$t0]=$t3] Store V[MaxIndex] to be the original V[n - 1]
